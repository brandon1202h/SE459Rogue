...
import se459rogue.assets.combat.Combat;
...
public class MonsterManager {
   private Combat combat = new Combat();
    ...
    private Monster selectMonster(int level){
        ...
        String lowerSymbolRangeMid = "BEHKORS";
    ...
    private void setStartingPostion(Monster monster, Room room){
        ...
        Hitbox monsterHitbox = new Hitbox(monsterPostion.getX(), monsterPostion.getY(), 16, 16);
        monster.setPosition(monsterPostion);
        monster.setHitbox(monsterHitbox);
    }

    public void moveMonster(Level level, PlayerManager pm){
        Random combatIniative = new Random();
        for(int x = 0; x < level.getNumberOfRooms(); x++){
            if(level.getRooms().get(x).getMonster() != null){
                if(!level.getRooms().get(x).getMonster().isDefeated()){
                    if(level.getRooms().get(x).getMonster().getPathfinding() == 0){
                        ...
                        pathFindingSeek(level.getRooms().get(x).getMonster().getPosition(), pm.getPosition(), level.getRooms().get(x));
                    }
                    combat.startCombat(pm, level.getRooms().get(x).getMonster(), combatIniative.nextInt(2));
                }
            }
        }
    }
    ...
    private void pathFindingSeek(Position start, Position destination, Room room){
        if( ( Math.abs( (start.getX() -1)  - destination.getX()) < Math.abs(start.getX() - destination.getX()))){
            ...
            room.getMonster().getHitbox().setPosition(room.getMonster().getPosition().getX(), room.getMonster().getPosition().getY());   
